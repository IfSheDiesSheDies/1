...
final class DefaultBotHandlers {
    ...
    private static func messageHandler(bot: TGBot) async {
        await bot.dispatcher.add(TGMessageHandler(filters: (.all && !.command.names(["/howtoshit", "/show_buttons"]))) { update in
    ...
    private static func commandPingHandler(bot: TGBot) async {
        await bot.dispatcher.add(TGCommandHandler(commands: ["/test1"]) { update in
            try await update.message?.reply(text: "reponse", bot: bot)
        })
    }
